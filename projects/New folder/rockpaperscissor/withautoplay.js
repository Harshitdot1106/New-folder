let score = JSON.parse(localStorage.getItem('score')) || {
 ...
 function autoplay(){
    if(isAutoplay){
   inter =  setInterval(()=>{
        const playerMo=pickComputerMove()
      playGame(playerMo);
 ...
 /*when the buton is pressed frist time the if statement runs but the boolean value
 returns to the false value,nut the autoplay keeps running whem the button is pressed again 
 now the boolean value is false so the else part runs and the loop stops */
